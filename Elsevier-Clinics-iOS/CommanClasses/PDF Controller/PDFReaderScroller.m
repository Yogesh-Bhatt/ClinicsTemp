...
-(void)setScrollerZoomScal
{
	...
	//[NSTimer scheduledTimerWithTimeInterval:0.2 target:self selector:@selector(checkForOrientation) userInfo:nil repeats:NO];
}
-(void)checkForOrientation
{
//	ElsevierAppDelegate *appDelegate=(ElsevierAppDelegate*)[[UIApplication sharedApplication]delegate];
...
-(void)setZoomForLandscape
{
	...
	//NSLog(@"Wdith %f",self.contentSize.width);
...
-(void)setContaintOffset
{
		...
		//if(doubleTabScale.x/2.0>self.contentSize.width/2)
//		{
//			[self setContentOffset:CGPointMake(pdfView.frame.size.width/2, self.contentOffset.y)];
//		}
//		else
//		{
...
- (BOOL)touchesShouldBegin:(NSSet *)touches withEvent:(UIEvent *)event inContentView:(UIView *)view {
	////NSLog(@"Touch Should Begin");
	
	if ([[event allTouches] count] > 1) return NO;
	
	UITouch *touch = [touches anyObject];
	CGPoint currentPoint = [touch locationInView:self];
	touchStartX = currentPoint.x;
	
	////NSLog(@"Touch Start: %f", touchStartX);
...
{
	self.zoomScale=1.0;
	self.minimumZoomScale=1.742;//self.frame.size.width/pdfView.frame.size.width;
	self.zoomScale=1.742;//self.frame.size.width/pdfView.frame.size.width;
	[self setContentOffset:CGPointMake(0.0, self.contentOffset.y)];
