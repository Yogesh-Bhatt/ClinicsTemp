    //
//  BookMarksDetailsViewController_iPad.m
//  Clinics
//
...
@implementation BookMarksDetailsViewController_iPad
    ...
    switch (indexPath.row)
    {
        ...
        {    
			...
			if ([m_arrArticles count] == 0) {
				...
				((ClinicDetailHeaderCellView *)cell).m_lblClinicTitle.frame=CGRectMake(125, 19, 588, 42);
				((ClinicDetailHeaderCellView *)cell).m_lblClinicTitle.text = @"To add bookmarks, tap the star that appears with any article.";
...
- (void)showPopOver
{
    [self dismissPopoover];
	ClinicsAppDelegate  *appDelegate=(ClinicsAppDelegate *)[UIApplication sharedApplication].delegate;
	id  viewConroller=nil;
	if(appDelegate.m_nCurrentTabTag == kTAB_CLINICS){
		viewConroller = [[ClinicListViewController alloc] initWithNibName:@"ClinicListViewController" bundle:nil];
		
	}
	else if(appDelegate.m_nCurrentTabTag == kTAB_BOOKMARKS){
		viewConroller = [[BookMarkListViewController_iPad alloc] initWithNibName:@"BookMarkListViewController_iPad" bundle:nil];
		
	}
	else if(appDelegate.m_nCurrentTabTag == kTAB_NOTES){
        
		viewConroller = [[NotesListViewController_iPad alloc] initWithNibName:@"NotesListViewController_iPad" bundle:nil];
	...
	[viewConroller initClinicListView];
	[viewConroller setFrameM_ScrollView];
    RELEASE(viewConroller);
